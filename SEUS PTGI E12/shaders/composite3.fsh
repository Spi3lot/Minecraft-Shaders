...
#include "lib/GBufferData.inc"
 ...
 void main()
 {
   ...
   vec3 y=pow(texture2DLod(colortex3,texcoord.xy,2).xyz,vec3(2.2)),z=GetViewPosition(texcoord.xy,GetDepth(texcoord.xy)).xyz,s=GetNormals(texcoord.xy);
   float m=pow(1.-saturate(dot(-normalize(z),s)),5.);
   v.xyz*=m;
