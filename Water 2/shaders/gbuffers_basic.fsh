...
/*
float sdf(vec3 p) {
    vec3 pos = (fs_in.glvertex.xyz + cameraPosition) / WAVE_SIZE;
    vec3 offset = WAVE_SPEED * vec3(frameTimeCounter, 0, 0);
    float displacement = WAVE_AMPLITUDE * simplex3d(pos + offset) + WAVE_DISPLACEMENT;

    return p.y - displacement;
}
...
void main() {
    ...
    if (fs_in.entity.x == 9) {
        ...
        vec3 rd = normalize(fs_in.viewPos.xyz);

        for (int i = 0; i < 50; i++) {
            float dist = ro.y - displacement;
